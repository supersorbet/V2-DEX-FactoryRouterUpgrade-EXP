...
abstract contract RouterProtecc {
    ...
    function _getReserves(address pair, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (uint reserve0, uint reserve1) = IPair(pair).getReservesSimple();
    ...
    function _swap(address initialPair, uint[] memory amounts, address[] memory path, address to) internal { // not in use
        for (uint i; i < path.length - 1; i++) {
            ...
            address currentTo = i < path.length - 2 ? Library.pairFor(_factory, output, path[i + 2]) : to;

            // perfrom the swap, ingredient tokens have already transferred by the
            // last sub swap with `to` or its caller function.
            address pair = i == 0 ? initialPair : Library.pairFor(_factory, input, output); // use initial pair;
            ILiquidityPair(pair).swap(amount0Out, amount1Out, currentTo, new bytes(0));
        }
    }
    */
    // requires the initial amount to have already been sent to the first pair
    function _swapMemoryGasSipping(address _factory, address initialPair, uint[] memory amounts, address[] calldata path, address to) internal {
        ...
        for (uint i; i < path.length - 1; ) {
            ...
            if (i < path.length - 2) {
                ...
                nextPair = Library.pairFor(_factory, output, path[i + 2]); // cache `to` as `nextPair` for the next sub swap.
                // execute swap,tokens have already transferred by the
                // last sub swap with `to` or its caller function
                _swapSingle(pair, amountOut, input, output, nextPair);
            } else {
                // `to` is the sender
                // execute swap, tokens already transferred by the
                // last sub swap with `to` or its caller function.
    ...
    function _swapSingle(address pair, uint amountOut, address tokenIn, address tokenOut, address to) internal {
        if (tokenIn < tokenOut) { // whether input token is `token0`
            IPair(pair).swapFor1(amountOut, to);
        } else {
            IPair(pair).swapFor0(amountOut, to);
        }
    }
}
